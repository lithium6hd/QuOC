...
class SpeedAndAtomNumber(FoM):
    """
    Estimator for normal distributed variable. Returnes the deviation / mean as a figure of merit.
    """

    def __init__(self, number_of_samples=50, min_atom_number=1000, s_by_atoms=1e-5):
        # self.acc = accuracy
        ...
        self.min_atom_number = min_atom_number
        self.s_by_atoms = s_by_atoms
        self.atom_number_figure_of_merit = 0
    ...
    def update(self, target_value: float) -> bool:
        ...
        if self.estimate < self.min_atom_number:
            self.atom_number_figure_of_merit = 1e6 + (self.min_atom_number - self.estimate)**2
        else:
            self.atom_number_figure_of_merit = self.s_by_atoms * (self.estimate - self.min_atom_number)
        
        
        self.atom_number_figure_of_merit
    ...
    def get(self, parameters) -> float:
        sequence_time = parameters['SequenceTime'] # Todo: fix how to get the sequence duration
        fom = sequence_time - self.atom_number_figure_of_merit
        return fom
